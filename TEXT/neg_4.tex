...
For example, the spectrum in the thermodynamic limit was found in \cite{Brouwer}.
An equation for the position of the eigenvalues in the complex plane and the density of states was found in \cite{Goldsheid}.
...
The "Hatano-Nelson" model describes vortex depinning in superconductors \cite{Hatano1,Hatano2} ,certain types of DNA denaturation \cite{DNA1,DNA2} and non conservative population biology \cite{popbio}.
...
Conservative processes: Molecular motors \cite{brm1,brm2}, fluid dynamics \cite{fluid}.
...
%
\be{17}
\text{Re}[\lambda] \geq \frac{2 \sinh^2(\mathcal{S}/2)}{g^2} \left[ 1 + \sqrt{1- \frac{g^2 \mathcal{S}^2}{4\sinh^2(\mathcal{S}/2)}}\right] \ \ =  \ \ \Delta(s)
\ee
%
...
\begin{figure}[h]
\includegraphics[height=2.5cm]{/Figs/N_E_1_5}
\includegraphics[height=2.5cm]{/Figs/N_E_2_5}
\includegraphics[height=2.5cm]{/Figs/N_E_3_5}
\includegraphics[height=2.5cm]{/Figs/N_E_4_5}
\includegraphics[height=2.5cm]{/Figs/N_E_5_5}

\includegraphics[height=2.5cm]{/Figs/N_E_1_4}
\includegraphics[height=2.5cm]{/Figs/N_E_2_4}
\includegraphics[height=2.5cm]{/Figs/N_E_3_4}
\includegraphics[height=2.5cm]{/Figs/N_E_4_4}
\includegraphics[height=2.5cm]{/Figs/N_E_5_4}

\includegraphics[height=2.5cm]{/Figs/N_E_1_3}
\includegraphics[height=2.5cm]{/Figs/N_E_2_3}
\includegraphics[height=2.5cm]{/Figs/N_E_3_3}
\includegraphics[height=2.5cm]{/Figs/N_E_4_3}
\includegraphics[height=2.5cm]{/Figs/N_E_5_3}

\includegraphics[height=2.5cm]{/Figs/N_E_1_2}
\includegraphics[height=2.5cm]{/Figs/N_E_2_2}
\includegraphics[height=2.5cm]{/Figs/N_E_3_2}
\includegraphics[height=2.5cm]{/Figs/N_E_4_2}
\includegraphics[height=2.5cm]{/Figs/N_E_5_2}

\includegraphics[height=2.5cm]{/Figs/N_E_1_1}
\includegraphics[height=2.5cm]{/Figs/N_E_2_1}
\includegraphics[height=2.5cm]{/Figs/N_E_3_1}
\includegraphics[height=2.5cm]{/Figs/N_E_4_1}
\includegraphics[height=2.5cm]{/Figs/N_E_5_1}
\caption{Integrated density of states.
The blue points are calculated from numerical diagnoalization. 
The red line dashed curve corresponds to the log-box result (large $s$), \Eq{e92}.
The black dashed-dot line is \Eq{e93} and the green line is just a free particle \Eq{e94}.
...
%
\beq
C \equiv \frac{1}{2}e^{-s/2}\cosh\left(\frac{\sigma}{2}\right) 
\eeq
%
%
...
&\approx & \int_a^b \left(\ln(X) -\frac{x}{X} + \frac{1}{2} \frac{y^2}{X^2} + \frac{xy^2}{X^3}\right)\rho(X) dX = \\
...
%
\be{96}
\Delta_{\lambda} \ = \ 2\pi^2 \frac{C_2}{|C_1|^3} 
\sim \frac{1}{N^2}\frac{(\mu-1)^3}{\mu^2(\mu-2)}
\frac{1- \left(\frac{a}{b}\right)^{\mu-2}}{\left(1- \left(\frac{a}{b}\right)^{\mu-1}\right)^3}b^{1-2\mu}
\ee
%
the last relation is for  $\rho(x) \sim N\mu x^{\mu-1}$.
\rmrk{I'm not sure about the limit of the integral}
%

\subsect{Condition for complexity}

For the general density of states 
%
\beq
\mathcal{N}(x) = \left(\frac{x}{x_c}\right)^{\mu}
\eeq
%
The electrostatic potential along the real axis is (integration by parts)
%
\beq
V(R) &=& -  \int_0^{x_c} \frac{\mathcal{N}(x)}{x-R}dx
\eeq
%
The complexity is determined by the slope of the potential at the origin $V(+0)$.
%
The slope is (derivative with respect to $R$ and integrating by parts)
%
\beq
V'(R) &=& \int_0^{x_c} \frac{\mathcal{N'}(x)}{x-R}dx = \frac{\mu}{x_c^{\mu}}\int_0^{x_c} \frac{x^{\mu-1}}{x-R}dx \ = \ \frac{C}{x_c}
\eeq
%
where $C=x_c V'(R)$. This integral converges for $\mu < 1$. We make the substitution $z=x/R$ under which 
%
\beq
...
\beq
I &=& \int_0^{u}\frac{x^{\mu-1}}{x-1}dx =\lim_{\varepsilon \to 0} 
\left[ \int_0^{1-\varepsilon}x^{\mu-1}(1-x)^{-1} + 
\int_{1+\varepsilon}^u x^{\mu-1}(1-x)^{-1} \right]= \\
...
%
Two go from the first to second line we made the substitution $t=1/x$ and took the limit $u\to \infty$.
%
It is clear that $C$ and thus $V'(+0)$ changes sign from positive to negative at $\mu=1/2$.
%
\beq
V'(R) =\mu \frac{ R^{\mu}}{x_c^{\mu}} \pi \cot(\pi \mu)
\eeq
%
...
This means we are interested in the region $s \gg s_{\infty}$.
The eigenvalues have a $\rho\sim1/x$ distribution and the inverse localization length, $\kappa(x)$ is given by \Eq{e91}.
Define $x_c$ such that the eigenvalues in the range $\exp[(s-\sigma)/2] < x < x_c$ are complex, $x_c$ is determined by the equation
%
\beq
\kappa(x_c) = \frac{s}{2}
\eeq
...
The method described here appears in various forms in \cite{det1,Shnerb1}, it is known as the "Hatano - Nelson" method.
...
%L.G. Molinari, Determinants of block tridiagonal matrices, Linear Algebra and its Applications, Volume 429, Issues 8Ð9, 16 October 2008, Pages 2221-2226}
%
%\bibitem{det2}
